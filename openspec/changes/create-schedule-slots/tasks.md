...
### T6 — Run Pint
- [x] Run `vendor/bin/pint app/Filament/Resources/DoctorSchedules/ app/Models/DoctorSchedule.php database/factories/DoctorScheduleFactory.php` to fix style

---

### T7 — Write tests for `DoctorScheduleResource`
- [x] Run `php artisan make:test --pest Filament/DoctorScheduleResourceTest --no-interaction`
- [x] Write `beforeEach`: create `admin` role, create+assign admin user, `actingAs`
- [x] Write standard 5 tests: list renders, create renders, can create, edit renders, can edit
- [x] Write overlap test: create a slot, attempt to create overlapping slot for same doctor+day, assert no second record was created (or assert notification sent)
- [x] Write non-overlap test: slots on different days for same doctor are allowed
- [x] Run `php artisan test --compact --filter=DoctorScheduleResourceTest` and confirm all pass
