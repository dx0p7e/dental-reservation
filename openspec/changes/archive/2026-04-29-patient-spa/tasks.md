...
- [x] Run `npm run dev` inside `patient-spa/`
- [x] Visit `http://localhost:5173/` — confirm doctor cards and service list load
- [x] Register a new patient account, confirm redirect to `/appointments`
- [x] Log out, log back in
- [x] Pick a doctor → select slot + service → book → confirm appointment appears in list
- [x] Cancel the appointment, confirm status changes to `cancelled`
- [x] Visit `/loyalty` — confirm points balance displays
