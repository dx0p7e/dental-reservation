...
- **WHEN** the patient navigates to the booking page
...
The system SHALL display an inline empty-state message when no slots are available for the selected service: "No slots available for this service — submit a request instead" with a `<RouterLink>` to `/request-appointment?service_id={id}` styled as `text-clinic-blue underline`.

#### Scenario: Empty state shown when no slots
- **WHEN** the slot list for the selected service is empty
- **THEN** the inline empty-state message with the request-appointment link is visible instead of the grid
