...
```
{ doctor: { id: number; name: string; specialization: string }, slot: { id: number; date: string; start_time: string } | null, service: { id: number; name: string; price: string; loyalty_discount_pct: number | null } | null, storedAt: string }
```
